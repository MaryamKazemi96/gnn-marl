...
def main():
    ...
    parser.add_argument("--backbone", type=str, default=None,
                         choices=["sage", "dummy"],
                         help="Override config's backbone without editing the yaml — 'sage' is "
                              "the GNN (message-passing) backbone, 'dummy' is the non-graph MLP "
                              "ablation (see DummyBackbone in actor_critic.py). Lets ONE shared "
                              "config file serve both GNN and MLP runs from a shell script, "
                              "instead of maintaining two near-duplicate config files.")
    parser.add_argument("--data-dir", type=str, default=None,
                         help="Override config's data_dir without editing the yaml — for "
                              "pointing the SAME config at a different scenario's dataset "
                              "from a shell script (e.g. data/scenario_corridor/instance_1), "
                              "instead of generating a temporary override config file per run.")
    ...
    if args.backbone is not None:
        ...
        print(f"Overriding backbone -> {args.backbone}\n")
