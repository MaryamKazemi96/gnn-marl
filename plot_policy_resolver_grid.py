...
def gather_ppo_for_resolver(run_id: str, runs_root: Path) -> dict:
    """{"PPO_DET": {...}, "PPO_STOCH": {...}}, pulling eval_ppo.py's
    deterministic.json/stochastic.json from the SAME seed_dir/eval_results/
    folder the baselines write into — meaningful ONLY if PPO was actually
    trained and evaluated under THIS resolver (run_id), i.e. this run_id
    must include a real PPO training run, not just a baseline-only sweep.
    Aggregates reward_mean across seeds by averaging each seed's own
    reward_mean, since PPO eval files store per-seed stats, not raw
    reward lists."""
    run_root = runs_root / f"run_{run_id}"
    seed_dirs = sorted(run_root.glob("seed_*"))
    if not seed_dirs:
    ...
    for seed_dir in seed_dirs:
        ...
        if det_file.exists():
            try:
                ...
                det_means.append(d["stats"]["reward_mean"])
            except (KeyError, json.JSONDecodeError):
                pass
        if sto_file.exists():
            try:
                s = json.loads(sto_file.read_text())
                sto_means.append(s["stats"]["reward_mean"])
            except (KeyError, json.JSONDecodeError):
                pass

    out = {}
    if det_means:
        out["PPO_DET"] = {"reward_mean": float(np.mean(det_means)), "reward_std": float(np.std(det_means))}
    if sto_means:
        out["PPO_STOCH"] = {"reward_mean": float(np.mean(sto_means)), "reward_std": float(np.std(sto_means))}
    return out
...
def main():
    ...
    ap.add_argument("--output", type=str, default="policy_resolver_comparison.png")
    ap.add_argument("--title", type=str, default="Reward by policy and resolver")
    ap.add_argument("--include-ppo", action="store_true",
                     help="Also pull PPO_DET/PPO_STOCH from each resolver's own "
                          "eval_ppo.py results (deterministic.json/stochastic.json in "
                          "the same seed_dir/eval_results/ baselines use), so PPO is "
                          "compared against every baseline under the SAME resolver/run "
                          "it was trained with. Requires that each --resolver run_id "
                          "was actually a PPO training run, not just baselines.")
    args = ap.parse_args()
    ...
    for name, run_id in pairs:
        ...
        baselines = gather_baselines_for_resolver(run_id, runs_root)
        if args.include_ppo:
            ppo = gather_ppo_for_resolver(run_id, runs_root)
            if not ppo:
                print(f"  ⚠️  --include-ppo set but no PPO eval results found for "
                      f"resolver={name} — was eval_ppo.py --all-seeds run for this "
                      f"run_id? (train_ppo.py alone isn't enough — it needs a "
                      f"completed eval_ppo.py pass too.)")
            baselines.update(ppo)
        policy_data[name] = baselines
        print(f"  policies found: {sorted(baselines.keys())}")
