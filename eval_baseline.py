...
def load_run_config(run_dir: Path, fallback_config_path: str, conflict_resolution_override: str = None):
    ...
    if run_config is None:
        print(f" No run_metadata.json config found for {run_dir} — falling back to "
...
def main() -> None:
    ...
    ap.add_argument("--data-dir", type=str, default=None,
                    help="Path to generated data directory. If not given, reads "
                         ...
                         "to 'data' if neither is set) — no separate flag needed "
                         "as long as the config's data_dir points where you want.")
    ap.add_argument("--episodes", type=int, default=20,
                    help="Episodes per policy per seed")
    ...
    try:
        agents, tasks_batches = load_generated_data(resolved_data_dir)
        print(f"✓ Data loaded!")
        print(f"  Robots: {len(agents)}")
        print(f"  Batches: {len(tasks_batches)}")
        print(f"  Tasks: {sum(len(b) for b in tasks_batches)}\n")
