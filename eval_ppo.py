...
def main():
    ...
    ap.add_argument("--data-dir", default=None,
                     help="Path to generated data directory. If not given, reads "
                          "data_dir directly from --config's own YAML (falls back "
                          "to 'data' if neither is set).")
    ...
    # Resolve data_dir: explicit --data-dir wins; otherwise read it
    # directly from --config's own data_dir field — a single config
    # pointed at a specific scenario's data is enough on its own.
    if args.data_dir is not None:
        resolved_data_dir = args.data_dir
    else:
        _cfg_for_data_dir = load_config(args.config) or {}
        resolved_data_dir = _cfg_for_data_dir.get("data_dir", "data")
 
    print("\n============================")
    print(" PPO EVALUATION")
    print("============================\n")
 
    agents, tasks = load_data(resolved_data_dir)
