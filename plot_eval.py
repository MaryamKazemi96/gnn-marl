...
def load_baselines_all(baseline_file: Path) -> Dict[str, Dict]:
    ...
    out: Dict[str, Dict] = {}
    # Read every policy actually present in the file rather than a
    # hardcoded ["random", "greedy", "unique"] allow-list — that list
    # predates pickup_deadline/pickup_deadline_distance (and any future
    # new baseline) and would silently drop them from every downstream
    # plot without raising any error.
...
def plot_eval_rewards_per_episode(
    ...
    "GREEDY": "#f39c12",
    "UNIQUE": "#8c564b",
    "PROPOSAL_JOINT_COMPETITION": "#9498db",
    "PREDICTED_REWARD_JOINT": "#7ecc71",
    "PREDICTED_REWARD": "#9b59b6",
}
...
def plot_training_rewards(
    ...
    "GREEDY": "#f39c12",
    "UNIQUE": "#8c564b",
    "PROPOSAL_JOINT_COMPETITION": "#3498db",
    "PREDICTED_REWARD_JOINT": "#2ecc71",
    "PREDICTED_REWARD": "#9b59b6",
}
    for name, b_data in baselines.items():
        rewards = np.asarray(b_data.get("rewards", []), dtype=float)
