    ...
    for i in range(R):
        ...
        for local_slot, task_id in enumerate(cands):
            ...
            for src, dst in [(0, node_id), (node_id, 0)]:
                ...
                if edge_attr is not None and use_edge_rt and rid is not None:
                    try:
                        # BUG FIX: was `feature_fn(rid, t, "edge_rt")` — t is
                        # the full task DICT (from `t = tasks.get(task_id)`
                        # above), but the "edge_rt" dispatch branch in
                        # feature_fn.py does `task_id = str(obj_b)`,
                        # expecting a task ID STRING. Passing the dict
                        # meant str(obj_b) stringified the entire dict
                        # object into a garbled lookup key that never
                        # matches a real task, silently producing wrong
                        # (not the intended dx/dy/eta) values — confirmed
                        # directly: passing the dict vs. the correct
                        # task_id string for the same real robot/task pair
                        # produced completely different, non-corresponding
                        # feature values. This is very likely why a real
                        # training run's edge-feature sanity check showed
                        # near-zero variance even after correctly
                        # excluding padding via edge_mask.
                        edge_attr[i, e_ptr, :] = feature_fn(rid, task_id, "edge_rt")
                        if ego_edge_idx is not None:
                            edge_attr[i, e_ptr, ego_edge_idx] = 1.0
                    except Exception:
                        pass
            ...
            if two_hop:
                ...
                for j, other_rid in enumerate(robots):
                    ...
                    if e_ptr < E_max:
                        ...
                        if edge_attr is not None and use_edge_rt:
                            try:
                                edge_attr[i, e_ptr, :] = feature_fn(other_rid, task_id, "edge_rt")  # same fix as the ego-edge case above — t was the task dict, not the id string
                                if ego_edge_idx is not None:
                                    edge_attr[i, e_ptr, ego_edge_idx] = 0.0
                            except Exception:
                                pass
