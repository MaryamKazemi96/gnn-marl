...
class RTGNNPolicy(ActorCriticPolicy):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.gnn_ac = EgoActorCritic(
            ...
            eta_index=int(eta_index),
            lambda_init=float(lambda_init),
            noop_mode=cast(Literal["scalar", "state_dependent"], self.noop_mode),
            noop_init=float(noop_init),
            freeze_noop_logit=bool(freeze_noop_logit),
            **gnn_kwargs,
        )

        # Scalar noop_logit is ALWAYS created (backward compatible) but is
        # only actually READ in _append_noop_and_mask when
        # self.noop_mode == "scalar". In "state_dependent" mode, the
        # per-robot value comes from self.gnn_ac.noop_head instead.
        self.noop_logit = nn.Parameter(
            th.tensor(float(noop_init), dtype=th.float32),
            requires_grad=(not bool(freeze_noop_logit)),
        )
    ...
    def get_distribution(self, obs):
        ...
        logits_k, _, noop_logits_br = self._build_batch_outputs(obs_b)
        cand_mask = obs_b["cand_mask"]
        ...
        return self._dist_from_logits_flat(logits_flat)
    def _build_batch_outputs(self, obs_b: Dict[str, th.Tensor]) -> Tuple[th.Tensor, th.Tensor, Optional[th.Tensor]]:
        """
        Run EgoActorCritic over each batch element.
        ...
        Returns:
          logits_k: [B, R, K] candidate-only logits
          values:   [B, 1]
          noop_logits_br: [B, R] state-dependent noop logits, or None if
              self.noop_mode == "scalar".
        """
        any_tensor = next(iter(obs_b.values()))
        B = int(any_tensor.shape[0])
        ...
        for b in range(B):
            obs_one = {k: v[b] for k, v in obs_b.items()}  # drop batch dim => [R,...]
            logits_rk, value, noop_logit_r = self.gnn_ac(obs_one)   # logits: [R,K], noop_logit_r: [R] or None
            if logits_rk.dim() != 2 or logits_rk.shape[0] != self.R or logits_rk.shape[1] != self.K:
                raise RuntimeError(f"EgoActorCritic must return logits [R,K]={self.R,self.K}, got {tuple(logits_rk.shape)}")

            logits_list.append(logits_rk)
            if noop_logit_r is not None:
                noop_logits_list.append(noop_logit_r)
        ...
        if self.logit_temperature and self.logit_temperature != 1.0:
            logits_k = logits_k / float(self.logit_temperature)
        values = th.stack(values_list, dim=0).unsqueeze(-1)  # [B,1]
        noop_logits_br = th.stack(noop_logits_list, dim=0) if noop_logits_list else None  # [B,R] or None
        return logits_k, values, noop_logits_br

    def _append_noop_and_mask(
        self,
        logits_k: th.Tensor,          # [B,R,K]
        cand_mask: th.Tensor,         # [B,R,K] bool/int
        noop_logits_br: Optional[th.Tensor] = None,   # [B,R], from state_dependent mode
    ) -> Tuple[th.Tensor, th.Tensor]:
        """
        Returns:
          logits_full: [B,R,K+1]
        ...
        if self.noop_mode == "state_dependent" and noop_logits_br is not None:
            noop_col = noop_logits_br.unsqueeze(-1)              # [B,R,1]
        else:
            noop_col = self.noop_logit.expand(B, R, 1)           # [B,R,1] — original scalar behavior
        logits_full = th.cat([logits_k, noop_col], dim=-1)  # [B,R,K+1]
    ...
    def forward(self, obs, deterministic=False):
        ...
        logits_k, values, noop_logits_br = self._build_batch_outputs(obs_b)      # [B,R,K], [B,1], [B,R] or None
        cand_mask = obs_b["cand_mask"]                            # [B,R,K]
        ...
        # )
        #------------debug for candidate mask---------------
        logits_full, mask_full = self._append_noop_and_mask(logits_k, cand_mask, noop_logits_br)
        # pribt debug
        # probs = th.softmax(logits_full, dim=-1)
        # print(logits_full[0], '====================')
        # print(probs[0], '====================')
    ...
    def evaluate_actions(self, obs, actions):
        ...
        logits_k, values, noop_logits_br = self._build_batch_outputs(obs_b)
        #----debug for actor weights--------------
        # if not hasattr(self, "_debug_counter"):
        #     self._debug_counter = 0
    ...
    def predict_values(self, obs: Any) -> th.Tensor:
        ...
        _logits_k, values, _noop_logits_br = self._build_batch_outputs(obs_b)
        return values
    ...
    def _get_masked_logits(self, obs: Any) -> th.Tensor:
        ...
        logits_k, _values, noop_logits_br = self._build_batch_outputs(obs_b)      # [B,R,K]
        cand_mask = obs_b["cand_mask"]                            # [B,R,K]
        logits_full, mask_full = self._append_noop_and_mask(logits_k, cand_mask, noop_logits_br)  # [B,R,K+1]
        logits_full = logits_full.masked_fill(~mask_full, -1e9)
        return logits_full
